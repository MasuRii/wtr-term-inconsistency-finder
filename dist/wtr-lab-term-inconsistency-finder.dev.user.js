...
// @version 5.3.7-dev.1762901454426
// @author MasuRii
...
__webpack_require__.d(__webpack_exports__, {
  Jf: () => (/* reexport */ applySmartQuotesReplacement),
  sz: () => (/* reexport */ applyTermReplacements),
  bn: () => (/* reexport */ core/* crawlChapterData */.bn),
  ZD: () => (/* reexport */ escapeHtml),
  Nt: () => (/* reexport */ escapeRegExp),
  zF: () => (/* reexport */ core/* extractJsonFromString */.zF),
  Ir: () => (/* reexport */ core/* getNovelSlug */.Ir),
  mT: () => (/* reexport */ core/* isWTRLabTermReplacerLoaded */.mT),
  Rm: () => (/* reexport */ core/* log */.Rm),
  fN: () => (/* reexport */ summarizeContextResults),
  oV: () => (/* reexport */ validateResultForContext)
});

// UNUSED EXPORTS: areSemanticallySimilar, calculateResultQuality, mergeAnalysisResults, sanitizeResultsData, sanitizeSuggestionData

// EXTERNAL MODULE: ./src/modules/utils/core.js
...
function smartenQuotes(text) {
  ...
  const smartQuotesEnabled =
    state/* appState */.XJ?.config?.smartQuotesEnabled !== undefined
      ? Boolean(state/* appState */.XJ.config.smartQuotesEnabled)
      : true;
  ...
  try {
    ...
    (0,core/* log */.Rm)("SMART QUOTES STATS (smartenQuotes):", {
      originalStraightSingles,
      originalStraightDoubles,
      originalSmart,
      newStraightSingles,
...
function applySmartQuotesReplacement(chapterData) {
  ...
  (0,core/* log */.Rm)(`Applying smart quotes replacement to ${chapterData.length} chapters...`);
  ...
  const processedData = chapterData.map((data) => {
    ...
    if (anomaly) {
      ...
      (0,core/* log */.Rm)(`SMART QUOTES CONVERSION Chapter #${data.chapter}:`);
      (0,core/* log */.Rm)(
        `  Original: ${originalStraightQuotes} straight, ${originalSmartQuotes} smart`,
      );
      (0,core/* log */.Rm)(`  After:    ${newStraightQuotes} straight, ${newSmartQuotes} smart`);
      (0,core/* log */.Rm)(`  Converted: ${quotesConverted} quotes to smart format`);
      (0,core/* log */.Rm)(
...
function applyTermReplacements(chapterData, terms = []) {
  if (!terms || terms.length === 0) {
    (0,core/* log */.Rm)("No terms provided. Skipping replacement step.");
    return chapterData;
  }
  (0,core/* log */.Rm)(`Applying ${terms.length} replacement terms using advanced logic.`);
...
function summarizeContextResults(existingResults, maxItems = 50) {
  ...
  const sortedResults = existingResults
    .map((result) => ({
      ...result,
      qualityScore: results_calculateResultQuality(result),
    }))
...
function results_calculateResultQuality(result) {
...
// src/modules/utils/index.js
// Backward compatibility exports for utils.js
...
// src/modules/utils.js
// Legacy compatibility wrapper - now imports from modularized utils components
...
/* harmony import */ var _ui__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(201);
...
function scheduleRetriableRetry({
  ...
  (0,_ui__WEBPACK_IMPORTED_MODULE_2__/* .updateStatusIndicator */ .LI)(
    "running",
...
function handleApiError(errorMessage) {
  ...
  (0,_ui__WEBPACK_IMPORTED_MODULE_2__/* .updateStatusIndicator */ .LI)("error", "Error!");
  (0,_ui__WEBPACK_IMPORTED_MODULE_2__/* .displayResults */ .Hv)(_state__WEBPACK_IMPORTED_MODULE_0__/* .appState */ .XJ.runtime.cumulativeResults);
}
...
function handleApiErrorStatus(
  ...
  if (isRetriable) {
    ...
    if (errorStatus === "RESOURCE_EXHAUSTED") {
      ...
      (0,_utils__WEBPACK_IMPORTED_MODULE_1__/* .log */ .Rm)(`Key ${currentKeyIndex} on temporary COOLDOWN for 1 minute.`);
    } else if (errorStatus === "DEADLINE_EXCEEDED") {
      // Request timeout - brief cooldown
      const unlockTime = Date.now() + 30 * 1000; // 30 seconds
      (0,_state__WEBPACK_IMPORTED_MODULE_0__/* .updateKeyState */ .gH)(currentKeyIndex, "ON_COOLDOWN", unlockTime, 1);
      (0,_utils__WEBPACK_IMPORTED_MODULE_1__/* .log */ .Rm)(`Key ${currentKeyIndex} on timeout COOLDOWN for 30 seconds.`);
    }
...
/* harmony import */ var _state__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(907);
/* harmony import */ var _utils__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(9);
/* harmony import */ var _geminiApi__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(970);
...
/* harmony import */ var _state__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(907);
/* harmony import */ var _geminiApi__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(970);
...
function generatePrompt(chapterText, existingResults = []) {
  ...
  if (existingResults.length > 0) {
    ...
    const validResults = existingResults.filter((result) => {
      const isValid = (0,_utils__WEBPACK_IMPORTED_MODULE_0__/* .validateResultForContext */ .oV)(result);
      if (!isValid) {
    ...
    if (validResults.length === 0) {
      (0,_utils__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)("All existing results failed validation, proceeding without context");
    } else {
      (0,_utils__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)(
    ...
    const summarizedResults = (0,_utils__WEBPACK_IMPORTED_MODULE_0__/* .summarizeContextResults */ .fN)(validResults, 30); // Limit to 30 detailed items
...
/* harmony import */ var _utils__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(9);
/* harmony import */ var _ui__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(201);
/* harmony import */ var _keys__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(41);
/* harmony import */ var _prompts__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(322);
...
function findInconsistencies(
  ...
  const apiKeyInfo = (0,_keys__WEBPACK_IMPORTED_MODULE_3__/* .getAvailableApiKey */ .R)();
  if (!apiKeyInfo) {
    handleApiError(
  ...
  const prompt = (0,_prompts__WEBPACK_IMPORTED_MODULE_4__/* .generatePrompt */ .s)(combinedText, existingResults);
  const requestData = {
  ...
  GM_xmlhttpRequest({
    ...
    onload: function (response) {
      (0,_utils__WEBPACK_IMPORTED_MODULE_1__/* .log */ .Rm)("Received raw response from API:", response.responseText);
      let apiResponse;
      let parsedResponse;
      ...
      if (apiResponse.error) {
        const handled = (0,_keys__WEBPACK_IMPORTED_MODULE_3__/* .handleApiErrorStatus */ .j)(
          apiResponse,
          operationName,
          currentKeyIndex,
          retryCount,
      ...
      try {
        ...
        const cleanedJsonString = (0,_utils__WEBPACK_IMPORTED_MODULE_1__/* .extractJsonFromString */ .zF)(resultText);
      ...
      (0,_ui__WEBPACK_IMPORTED_MODULE_2__/* .updateStatusIndicator */ .LI)("complete", "Complete!");
      const continueBtn = document.getElementById("wtr-if-continue-btn");
      if (continueBtn) {
        continueBtn.disabled = false;
      }
...
/* harmony import */ var _utils_core__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(901);
...
async function loadConfig() {
  ...
  if (savedConfig.apiKey && !savedConfig.apiKeys) {
    (0,_utils_core__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)("Migrating legacy single API key to new array format.");
  ...
  if (savedConfig.preferences) {
    ...
    (0,_utils_core__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)("Loaded preferences from config:", appState.preferences);
  }
  ...
  if (sessionResults) {
    try {
      ...
      (0,_utils_core__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)(
      ...
      if (sanitizedResults.length !== rawResults.length) {
        (0,_utils_core__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)("🔧 Data sanitization: Results count changed during cleanup");
      } else {
      ...
      (0,_utils_core__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)("Failed to parse session results:", e);
    }
  }
}
...
function saveSessionResults() {
  try {
    ...
    (0,_utils_core__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)("Session results saved");
  } catch (e) {
...
function clearSessionResults() {
  try {
    ...
    (0,_utils_core__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)("Session results cleared");
  } catch (e) {
...
function getNextAvailableKey() {
  ...
  if (soonestKey !== null) {
    ...
    (0,_utils_core__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)("All available API keys are permanently invalid or exhausted.");
  }
...
__webpack_require__.d(__webpack_exports__, {
  ...
  Rq: () => (/* reexport */ keys/* getAvailableApiKey */.R)
});

// UNUSED EXPORTS: findInconsistencies, generatePrompt, handleApiErrorStatus

// EXTERNAL MODULE: ./src/modules/api/keys.js
var keys = __webpack_require__(41);
// EXTERNAL MODULE: ./src/modules/api/prompts.js
var prompts = __webpack_require__(322);
// EXTERNAL MODULE: ./src/modules/api/core.js
...
function findInconsistenciesDeepAnalysis(
  ...
  const contextResults =
    state/* appState */.XJ.runtime.cumulativeResults.length > 0
  ...
  if (targetDepth > 1) {
    ...
    Promise.resolve(/* import() */).then(__webpack_require__.bind(__webpack_require__, 526)).then(({ findInconsistencies }) => {
...
function findInconsistenciesIteration(
  ...
  const maxTotalRetries =
    Math.max(1, state/* appState */.XJ.config.apiKeys.length) * MAX_RETRIES_PER_KEY;
  let retryCount = 0;
  ...
  const executeIteration = () => {
    ...
    const apiKeyInfo = (0,keys/* getAvailableApiKey */.R)();
    if (!apiKeyInfo) {
      handleApiError(
        "All API keys are currently rate-limited or failing. Please wait a moment before trying again.",
      );
    ...
    const prompt = (0,prompts/* generatePrompt */.s)(combinedText, existingResults);
    const requestData = {
      contents: [{ parts: [{ text: prompt }] }],
      generationConfig: {
      ...
      onload: function (response) {
        ...
        if (apiResponse.error) {
          ...
          if (isRetriable) {
            ...
            state/* appState */.XJ.runtime.apiKeyCooldowns.set(
              currentKey,
        ...
        try {
          const resultText = candidate.content.parts[0].text;
          const cleanedJsonString = (0,utils/* extractJsonFromString */.zF)(resultText);
          parsedResponse = JSON.parse(cleanedJsonString);
          (0,utils/* log */.Rm)(
            `${operationName}: Successfully parsed API response content.`,
            parsedResponse,
        ...
        state/* appState */.XJ.runtime.currentApiKeyIndex =
          (currentKeyIndex + 1) % state/* appState */.XJ.config.apiKeys.length;
        ...
        if (isVerificationRun) {
          ...
          state/* appState */.XJ.runtime.cumulativeResults = (0,state/* mergeAnalysisResults */.bd)(
            state/* appState */.XJ.runtime.cumulativeResults,
            allNewItems,
          );
        } else {
...
// src/modules/api/index.js
// Backward compatibility exports for geminiApi.js
...
// src/modules/geminiApi.js
// Legacy compatibility wrapper - now imports from modularized API components
